  ...
  weapon: {
    ...
    options: {
      ...
      sniper: {
        label: 'Sniper',
        draw(ctx) {
          // Replicates the rifle's grip-frame so the existing rifle stance
          // (right_arm 1.35, right_forearm -1.50, left_arm -2.20, left_forearm 0.10)
          // applies cleanly. Visual extras: longer barrel, scope on top,
          // protruding bolt handle.

          // Barrel — longer than rifle's 58 to read as a precision rifle.
          ctx.fillStyle = '#1A1A1A';
          ctx.fillRect(-2, -8, 4, 84);    // barrel shaft

          // Receiver block (hand grips here)
          ctx.fillStyle = '#2E2E2E';
          ctx.fillRect(-5, -8, 10, 26);

          // Stock (butt end behind grip)
          ctx.fillStyle = '#7B5E2A';
          ctx.fillRect(-5, -34, 10, 28);
          ctx.fillRect(-8, -38, 16, 8);

          // Cheek riser on top of stock (sniper-specific)
          ctx.fillStyle = '#5C4317';
          ctx.fillRect( 5, -32, 4, 18);

          // Pistol grip
          ctx.fillStyle = '#5C3D12';
          ctx.fillRect(-3, 16, 6, 16);

          // Single-stack magazine (slimmer than the rifle's box mag)
          ctx.fillStyle = '#444';
          ctx.fillRect(-2, 6, 4, 12);

          // Bolt handle protruding to the +X side
          ctx.fillStyle = '#888';
          ctx.fillRect(5, 0, 6, 3);
          ctx.fillStyle = '#aaa';
          ctx.beginPath();
          ctx.arc(12, 1.5, 2, 0, Math.PI * 2);
          ctx.fill();

          // Scope mounted on top of the receiver (+X side)
          // Rear mount ring
          ctx.fillStyle = '#444';
          ...
          ctx.fillRect( 7, -3, 6, 16);
          // Front mount ring
          ctx.fillStyle = '#444';
          ctx.fillRect( 5, 13, 8, 3);
          // Objective bell
          ctx.fillStyle = '#222';
          ...
          // Bipod stub at front (folded)
          ctx.fillStyle = '#333';
          ctx.fillRect(-1, 52, 2, 8);

          // Muzzle brake
          ctx.fillStyle = '#111';
          ctx.fillRect(-3, 72, 6, 6);
          ctx.fillRect(-4, 74, 8, 4);
        },
      },
      grenade_launcher: {
